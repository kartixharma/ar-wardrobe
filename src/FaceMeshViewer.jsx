...
export default function FaceMeshViewer() {
  ...
  useEffect(() => {
    ...
    function getFaceAlignment(landmarks) {
      try {
        // --- UNCHANGED: Use same landmarks for position ---
        const leftEyeCenter = landmarks[159];
        const rightEyeCenter = landmarks[386];
        const noseBridge = landmarks[6];
        const noseTip = landmarks[1];

        // --- UNCHANGED: Position and Scale logic is the same ---
        const glassesCenter = {
        ...
        // This part is inside your original code and remains the same
        ...
        const world = uvToWorld(u, v, 0.0); // Assuming uvToWorld function is available
        ...
        // Select new landmarks from jaw, forehead, and chin
        const leftJaw = landmarks[132];
        const rightJaw = landmarks[361];
        const forehead = landmarks[10];
        const chin = landmarks[152];

        // Calculate yaw from the depth difference of jaw points
        const depthDifference = (leftJaw.z - rightJaw.z) / VIDEO_WIDTH;
        const yaw = -depthDifference * 4; // You can adjust '4' for sensitivity

        // Calculate pitch based on nose position relative to forehead/chin
        const pitch =
          ((noseTip.y - (forehead.y + chin.y) / 2) / VIDEO_HEIGHT) * 2; // You can adjust '2' for sensitivity
        ...
        if (
          ...
          if (predictions.length > 0) {
            ...
            if (face.keypoints && face.keypoints.length > 400) {
              ...
              setStatus(
                `👓 Glasses aligned with occlusion! Tracking ${predictions.length} face(s)`
              );
            }
          } else {
