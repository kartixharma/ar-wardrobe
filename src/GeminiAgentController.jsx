...
export default function GeminiAgentController() {
    ...
    const processRequest = useCallback(async (text, images) => {
        ...
        const requestBody = {
            contents: [{
                parts: [
                    { text: `This is a AR-Wardrobe app where user can try glasses, answer to the user as a agentic AI, The user said: "${text}" the response should be user friendly and short and simple without any formatting and suggest the glasses for user` },
            ...
            intervalId = setInterval(async () => {
                if (screenshotsRef.current.length < SCREENSHOT_COUNT) {
                    ...
                    if (screenshot) {
                        ...
                        setAgentStatus(`Captured ${screenshotsRef.current.length}/${SCREENSHOT_COUNT} images...`);
                    }
                } else {
    ...
    return (
        <div style={styles.agentContainer}>
            <p style={styles.agentStatus}>Agent Status: <span style={{ fontWeight: 'bold' }}>{agentStatus}</span></p>
            <button onClick={isRecording ? handleStop : handleStart} style={styles.micButton(isRecording)}>
                {isRecording ? <Square size={24} /> : <Mic size={24} />}
            </button>
            <div style={styles.transcriptionContainer}>
                <p><strong>You said:</strong> {transcript || "..."}</p>
                <p><strong>Gemini says:</strong> {aiResponse || "..."}</p>
            </div>
        </div>
    );
}
